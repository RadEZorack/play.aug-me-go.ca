      ...
      recognition.onresult = function(event) {
        ...
        for (var i = event.resultIndex; i < event.results.length; ++i) {
          if (event.results[i].isFinal) {
            ...
            final_span.innerHTML = linebreak(capitalize(final_transcript));
            recognizing = false;
            recognition.stop();
            // var utterThisPre = new SpeechSynthesisUtterance("Just a sec.");
            // synth.speak(utterThisPre);
            $.ajax({
              url: "/wolfram_spoken/",
              // url: "https://api.wolframalpha.com/v1/spoken",
              data: {
                appid: "YJVX2P-PHA77KP2E5",
                i: final_transcript
              },
              // url: "http://www.cleverbot.com/getreply",
              // data: {
              //   key: "CC9af4xJMJXTTRGcG3IRaMQYjUQ",
              ...
              success: function(data){
                ...
                // let text = data.getElementsByTagName("plaintext")[0].childNodes[0].nodeValue;
                // try{
                //   text += " " + data.getElementsByTagName("plaintext")[1].childNodes[0].nodeValue;
                // }catch{
                //   console.log("no second sentence")
                // }
                // console.log(text)
                // conversation_id = data.conversation_id
                final_span.innerHTML += linebreak("<br>"+text + ". ");
            ...
            image_result.src = "https://api.wolframalpha.com/v1/simple?appid=YJVX2P-PHA77KP2E5&i="+encodeURIComponent(final_transcript)
